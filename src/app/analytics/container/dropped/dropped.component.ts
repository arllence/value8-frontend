...
export class DroppedAnalyticsComponent implements OnInit {
  by_language = []
  by_gender = []
  by_age = []
  all_countries_data = []
  ...
  get_user_by_country(){
    ...
    this.administrationService.getrecords(by_country_url,payload).subscribe((res) => {
      if(res) {
        // console.log(res);
        this.all_countries_data =  res['data']
