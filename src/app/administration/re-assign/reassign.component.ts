...
import { list_user_roles, create_user_url, reassign_applications_url} from '../../app.constants';
  ...
  registeruser() {
    ...
    this.sweetalertService.showConfirmation('','Do You Wish to proceed?').then((res) => {
      ...
      if (res === false) {
        ...
        this.administrationService.postrecord(reassign_applications_url, payload).subscribe((res) => {
          if (res) {
            this.sweetalertService.showAlert('Success','Successful.','success');
